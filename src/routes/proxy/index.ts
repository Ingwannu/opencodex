...
function inferSupportsReasoning(modelId: string): boolean {
...
export function createProxyRouter(options: ProxyRoutesOptions) {
  ...
  async function proxyWithRotation(
    ...
    for (const candidate of routingCandidates) {
      ...
      for (let i = 0; i < attemptsForProvider; i++) {
        ...
        stripUnsupportedServiceTier(payloadToUpstream, candidate.provider);
  ...
  function toCodexModelCatalogShape(model: ExposedModel) {
    const contextWindow = model.metadata.context_window ?? 819_200;
    const autoCompactLimit = Math.min(240_000, Math.floor(contextWindow * 0.9));
    const priority = model.id === "gpt-5.5" ? 9 : model.id.includes("mini") ? 7 : 8;
    const supportsReasoning = model.metadata.supports_reasoning;
    const isGptModel = model.id.startsWith("gpt-");
    const serviceTiers = [
      {
        id: "priority",
        name: "Fast",
    ...
    return {
      ...
      description: `${model.metadata.provider} model ${model.id}`,
      ...
      comp_hash: `multicodex-${model.id}`,
      effective_context_window_percent: 95,
      experimental_supported_tools: model.metadata.supported_tool_types ?? [],
  ...
  router.get("/models", async (_req, res) => {
    ...
    res.json({
      object: "list",
      data: models.map(toOpenAiModelShape),
      models: models.map(toCodexModelCatalogShape),
    });
  });
